...
- 不强制任何国家/地域审美。
- 不强制中文提示词，按用户输入语言执行。
- 若用户明确指定风格（如国风、赛博朋克、水彩、写实），严格按用户要求生成。

## 输出目录与命名规范

| 项目 | 值 |
|------|------|
| **输出目录** | `outimage/ikunimage/`（工作区相对路径） |
| **扩展名** | `.png` |
...
```bash
python ~/.nanobot/skills/ikunimage/scripts/generate_ikun.py --setup
```
...
**可使用任意语言撰写**，结构：主体 + 场景 + 风格 + 光影 + 构图 + 细节约束

提示词模板：
```
[主体]：[人物/动物/物体主体描述]
[场景]：[环境与背景]
[风格]：[写实/动漫/水彩/油画/赛博朋克/国风/其他]
[元素]：[需要强调的元素或道具]
[光影]：[具体光影描述]
[色调]：[色彩倾向与氛围]
...
```
ikun_image(
  input_image="/path/to/original.jpg",
  prompt="编辑描述",
  aspect_ratio="3:4",
  retry=3
)
...
当用户需要一次生成多张图片时，使用 `batch_json` 参数并发批量生成。
...
ikun_image(
  batch_json='[
    {"prompt": "提示词内容1", "aspect_ratio": "3:4", "size": "2K", "output": "outimage/ikunimage/20260225_1430_主题_01.png"},
    {"prompt": "提示词内容2", "aspect_ratio": "3:4", "size": "2K", "output": "outimage/ikunimage/20260225_1430_主题_02.png"}
  ]',
  workers=2,
  retry=3
)
...
ikun_image(
  batch_json='[
    {"input": "/path/to/photo1.jpg", "prompt": "将背景改为雪景", "aspect_ratio": "3:4", "output": "outimage/ikunimage/20260225_1430_雪景编辑_01.png"}
  ]',
  workers=2,
  retry=3
)
...
| 参数 | 可选值 | 默认值 | 说明 |
|------|--------|--------|------|
| `prompt` | 图片描述文本 | 必填 | 提示词 |
| `aspect_ratio` | 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3, 21:9, 5:4, 4:5 | 1:1 | 宽高比 |
| `size` | 1K, 2K, 4K | 2K | 分辨率等级 |
| `output_path` | 文件路径 | 自动命名 | 输出路径 |
| `retry` | 0-10 | 3 | 最大重试次数 |
...
| 参数 | 可选值 | 默认值 | 说明 |
|------|--------|--------|------|
| `input_image` | 本地图片路径 | 必填 | 输入图片（< 4MB） |
| `prompt` | 编辑描述文本 | 必填 | 中文编辑描述 |
| `aspect_ratio` | 同上 | 1:1 | 输出宽高比 |
| `output_path` | 文件路径 | 自动命名 | 输出路径 |
| `retry` | 0-10 | 3 | 最大重试次数 |
...
| 参数 | 说明 |
|------|------|
| `batch_json` | JSON 数组字符串，每项含 prompt、aspect_ratio?、size?、output?、input? |
| `workers` | 并发数，默认 2，最大 8 |
| `retry` | 每个任务最大重试次数 |

> `prompt` 和 `batch_json` 互斥，必须二选一。
...
- "用另一张图" → 替换 input_image 参数

## 生成后交付

`ikun_image` 返回本地文件路径后：
- 若用户在飞书、Telegram 等聊天渠道且期望直接收到图片，用 `message` 工具的 `media` 字段发送文件。
- 否则返回文件路径，询问是否发送。
